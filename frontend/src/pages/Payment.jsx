...
const Payment = () => {
    ...
    const paymentHandler = async () => {
        ...
        if(method === "online"){
            const stripePromise = loadStripe('pk_test_51Slnv60scY6k57whAaER9ZY1CebeZch29gtV5aB4yDvgxz5rj7zscyhj6NpmzIb9ACC1PVWgCtaoI7D9z3MH6Afb0043Gk9tG4')
            // console.log(process.env.STRIPE_PUBLISHABLE_KEY)
          
            try {
                setLoading(true)
                const stripe = await stripePromise
                  console.log("before Hitting server")
                const {data} = await axios.post(`${server}/api/order/new/online`,{
                  ...
                  console.log("after Hitting server")
    ...
    <div>
        {
            ...
            <div className='Container mx-auto px-4 py-8'>
                <div className='space-y-8'>
                    ...
                    {
                        address && (
                            <div className='bg-card rounded-lg shadow border space-y-4 dark:border-gray-700'>
                    ...
                    <Button className='w-full py-3 mt-4' onClick={paymentHandler} disabled={!method || !address}>
                        Proceed to Checkout
                    </Button>
                </div>
            </div>
